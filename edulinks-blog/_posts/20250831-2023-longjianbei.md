...
翻一下HTTP流量包能看到一个 shell.jsp 的请求，看到有些流量都是加密过的。再往前面找看到一个请求中有个类似的 Key，也有 AES 的字样。这里有个知识点，AES 加密有多种模式，其中不需要 IV 变量的只有基础的 ECB 模式。

查询材料，可能可以利用 Godzilla的工具，简称哥斯拉。

第三问：该webshell的连接密码是多少？

/test.jsp?pwd=admin&cmd=ls%20/www/wwwroot/test.com/shell.jsp
