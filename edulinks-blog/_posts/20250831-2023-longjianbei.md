...
看别人的WP知道的，可以从 HTTP 对象中导出一个 huorong.exe 文件，执行后能程序文件所在的上级目录生成一个图片（所以比赛如果没有 Windows 环境很吃亏，发现这个图片在 Win 10 环境中可以正常打开，在 MacOs Monterey中就打不开），使用 010Editor 的 template 脚本能提示 CRC 校验错误，看 WP 是根据观察图片和经验把高度调整到 800，然后就能看到对应的 flag。
...
通过 `/etc/passwd` 和 `/etc/shadow` 查找，找最新的一条记录。
...
在 `idea` 目录下继续查找可疑的 mine_doge.sh 文件，里面能看到矿池地址和钱包地址。
...
还有一个办法可以不用写 Python 解密代码，使用 CyperChef 组合，之前有一道题，这类大概率是 AES 加密的 CBC 模式。

![image-20251011191809534](20250831-2023-longjianbei/image-20251011191809534.png)

WP 有个好的思路，是先把用户名 admin 加密后的字符串求出来，然后通过 http contains 查找对应的请求，再看里面的密码报文，根据这个方法可以找到一个 post 登录请求，解出密码就可以了。

第二问：app.config['SECRET_KEY']值为多少？

这个仔细找一下就能发现，使用 `http contains SECRET` 或者 `tcp contains SECRET`，有一个对首页的 GET 请求返回了相关内容。

第三问：flask网站由哪个用户启动？

这一问的 WP 没看明白，说是要找到带 whoami 命令执行的请求，但是截图看到并没有这个命令参数，这道题是利用 JWT cookie 解密发现更多信息。这里使用了一个 `flask-session-cookie-manager` 的开源 python 工具。

![image-20251015184347090](20250831-2023-longjianbei/image-20251015184347090.png)

第四问：攻击者写入的内存马的路由名叫什么？（答案里不需要加/）

这道题也是从 session 中解密出信息，可能得一个一个请求找，看了下状态码是200的请求不是很多。然后下面这个请求带了一个请求地址。

![image-20251015184016139](20250831-2023-longjianbei/image-20251015184016139.png)
