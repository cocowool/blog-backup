...
近几年Docker发展的趋势非常迅猛，通过百度指数我们能够看到，Docker的搜索量远远超过了Puppet。

![image-20200424175448678](puppet-introduce/image-20200424175448678.png)

通过使用Kubernetes编排调度我们在容器中的应用，仿佛Puppet已经没有存在的必要。

笔者认为，Docker肯定是未来发展的趋势，特别是结合微服务应用开发技术，容器平台的应用范围会越来越广。但是短期内，Puppet仍适用于传统运维的场景。两者应该是长期共存的关系，对于大型企业来说，如何组合使用好这两个工具，应该还要依靠各个企业根据自身的情况选择定制化的解决方案。

Docker的缺点是无法实现对物理服务器的批量配置管理。
